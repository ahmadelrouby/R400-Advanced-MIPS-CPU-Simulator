...
function tobin(s){
    ...
    var inst = "", reg1 = "", reg2 = "", reg3 = "", imm = "";
        ...
        if(inst != "j" && inst != "jal" && inst != "addi" && inst != "addiu" && inst != "andi" && inst != "ori" && inst != "xori" && inst != "slti" && inst != "sltiu" && inst != "lw" && inst != "sw" && inst != "beq" && inst != "bne" && inst != "ble" && inst != "add" && inst != "addu" && inst != "sub" && inst != "subu" && inst != "and" && inst != "or" && inst != "xor" && inst != "slt" && inst != "sltu" && inst != "sll" && inst != "srl" && inst != "sra" && inst != "jr" && inst != "jp"&& inst != "rp") //COPY THIS
        {
        ...
        if (s[x - 1] != '$'&&inst != "j" &&inst != "jal"  && inst!="jp" && inst!="rp")
        {
        console.log("Format of instruction: " + inst + " is not correct");
                return 0;
        }
        if (s.search(',') == -1 && inst != "j" && inst != "jal"&& inst != "jr"  && inst!="jp" && inst!="rp")
        {
        ...
        while (s[x] != ',' && inst!="j" && inst!="jal" && inst!="jp" && inst!="rp" && x<s.length)
        {
//        console.log("J or j Instruction 1");
        ...
        if ((reg1.length > 2 || reg1.length < 1 || parseInt(reg1) > 31) && inst != "j"&&inst != "jal"  && inst!="jp" && inst!="rp")
        {
...
//        console.log(s.substring(x))
        ...
        else
        {
                ...
                if (inst == "j" || inst == "jal"||inst=="jr"||inst=="jp"||inst=="rp")
                {
                        x= x-2;
                        reg2 = "0";
                        reg3 = "0";
...
//    console.log()
    var rs, rt, rd, shamt, funct,adr,immd;
        ...
        if (inst == "j" || inst == "jal" || inst == "jp"  || inst == "rp")
        {
                adr = (parseInt(imm));
                if (inst == "j")
                ...
                if(inst=="jp")
                        return adr | (parseInt("44")) << 26;
                else
                if(inst=="rp")
                        return adr | (parseInt("45")) << 26;
                return 0;
        }
        else
                return 0;
...
//        console.log("NOT STALLING RF");
        ...
        if(this.jump_now == 1 || this.jump_now == 2 || this.jump_now == 3 || this.jump_now == 4 || this.jump_now == 5 || this.flush_buffer == 1){
...
//            console.log("NORMAL JUMP: " + this.if_is_pc);
            ...
            this.if_is_pc=this.rf_forwardA; 
//            console.log("JUMP REGISTER: " + this.if_is_pc);
        } 
        else if(this.jump_now == 3 && this.pcSrc != 1)
        { 
            this.reg_file[31] = this.is_rf_pc;
            this.if_is_pc = adr * 4;
        } 
        else if(this.jump_now == 4 && this.pcSrc != 1)
        { 
            this.stack_procedure.push(this.is_rf_pc);
            this.if_is_pc = adr * 4;
        }  
        else if(this.jump_now == 5 && this.pcSrc != 1)
        { 
            if(this.stack_procedure.length > 0)
                this.if_is_pc = this.stack_procedure.pop();
            else
